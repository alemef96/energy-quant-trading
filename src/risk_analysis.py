...
def run_portfolio_risk_analysis():
    ...
    # Since we are modeling a power buyer or a short trader, risk lives in the high-price tail
    ...
    # 6. Embedded Quant Analysis Commentary
    print("\n================ QUANT ANALYST COMMENTARY ================")
    print(
        "1. THE MEAN REVERSION REGIME:\n"
        f"   The left chart shows that despite severe random price jumps, the spot price\n"
        f"   persistently reverts back to the structural production cost baseline (theta = {theta} €/MWh).\n"
        f"   This confirms the high mean-reversion speed parameter (kappa = {kappa}) is working.\n\n"
        "2. THE PITFALL OF TRADITIONAL VaR:\n"
        f"   The 95% Value at Risk (VaR) indicates that there is a 95% probability that the monthly\n"
        f"   settlement price will stay below {var_95:.2f} €/MWh. However, VaR tells us absolutely nothing\n"
        "   about the magnitude of losses when a tail event actually occurs.\n\n"
        "3. WHY CVaR / EXPECTED SHORTFALL IS VITAL FOR ENERGY DESKS:\n"
        f"   The 95% Expected Shortfall (CVaR) stands at {cvar_95:.2f} €/MWh. This means that in the 5% of\n"
        "   worst-case market scenarios (extreme physical asset outages, heatwaves, or supply shocks),\n"
        f"   the average price inside that red danger zone jumps to {cvar_95:.2f} €/MWh.\n"
        "   For a short trading book or a large industrial buyer, ignoring this difference can\n"
        "   lead to catastrophic under-capitalization and corporate default during market stress."
    )
